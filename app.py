...
def load_model():
    try:
        # Ensure the filename matches the saved file
        return joblib.load("xgb_model_fold_4.joblib")  # Update the filename accordingly
...
rotational_speed = st.number_input("Rotational speed [rpm]", min_value=0.0, max_value=5000.0, value=1601.0, step=1.)
...
type_l = st.selectbox("Type L (1 for Yes, 0 for No)", options=[0, 1], index=1, key="type_l")
type_m = st.selectbox("Type M (1 for Yes, 0 for No)", options=[0, 1], index=1, key="type_m")
